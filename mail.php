...
//	$result1=$database->query("SELECT * FROM ".TBL_USERS." WHERE username='$username'") or die(mysqli_error());
...
<body>
    <div class="container">
        ...
        if ($action == 'Send') {
            ...
            $q = sprintf("INSERT INTO mail (UserTo, UserFrom, Subject, Message, SentDate, status) VALUES ('%s','$session->username','%s','%s','%s','unread')", mysqli_real_escape_string($mailTo), mysqli_real_escape_string($subject), mysqli_real_escape_string($message), mysqli_real_escape_string($date));
        ...
        if ($action == "Inbox") {
            ...
            $q = sprintf("SELECT * FROM mail WHERE UserTo = '%s' ORDER BY SentDate DESC", mysqli_real_escape_string($user));
            $getMail = $database->query($q) or die(mysqli_error());
...
if ($action == "View") {
    ...
    $user = $session->username;
    $q = sprintf("SELECT * FROM mail WHERE UserTo = '%s' AND mail_id = '%s'", mysqli_real_escape_string($user), mysqli_real_escape_string($mail_id));
    $result = $database->query($q) or die(mysqli_error());
...
if ($action == 'Delete') {
    $id = $_POST['mail_id'];
    $query = sprintf("UPDATE mail SET 'Deleted' = 1 WHERE mail_id='%s' LIMIT 1", mysqli_real_escape_string($id));
